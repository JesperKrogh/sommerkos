...
│   ├── captions.md               # Bilingual captions for images
│   ├── optimist/
│   ├── j70/
│   ├── rsfeva/
│   └── ... (19 category folders)
...
1. **`images/`** — Raw uploaded images (READ-ONLY)
   ...
   - Each image can have an optional `.md` sidecar file for captions
...
2. **`scripts/generate_captions.py`** — Image processor
   ...
   - Generates bilingual captions using AI (Gemini 2.5 Flash Image)
   - Outputs to `images-web/`
...
3. **`images-web/`** — Processed images (served to users)
   ...
   - Captions stored in `images/captions.md` (processed alongside images)
